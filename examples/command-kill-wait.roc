...
main! = |_args| {
	...
	talker = leashed!("echo out-before-kill; echo sync >&2; sleep 100")?
	...
	complainer = leashed!("echo err-before-kill >&2; echo sync; sleep 100")?
	...
	racer = leashed!("echo raced; exit 7")?
	...
	noisy = leashed!("yes hello | head -c 500000; echo sync >&2; sleep 100")?
	_sync3 = noisy.read_stderr!(5)?
	loud = noisy.kill_wait!()?
	Stdout.line!("noisy: exit ${loud.exit_code.to_str()}, ${loud.stdout.len().to_str()} bytes")?

	# --- a leashed child takes its own children down with it ---
	forker = leashed!("sleep 100 & echo forked; sleep 100")?
...
leashed! : Str => Try(Cmd.Child, _)
leashed! = |script|
	Cmd.new_str("sh").args_str(["-c", script]).spawn_leashed!()
